...
# Here spending data will be a data frame
...
class TestTimeSeriesPredictions(unittest.TestCase):
    def setUp(self):
        ...
        self.category_data = {
            ...
            'transport': pd.DataFrame({
                'date': dates[10:25].copy(),
                'amount': np.random.randint(10, 30, size=15)
            }).copy(),
            'entertainment': pd.DataFrame({
                'date': dates[20:].copy(),
                'amount': np.random.randint(20, 40, size=10)
            }).copy()
        }
        
        # Edge cases
        self.empty_data = pd.DataFrame(columns=['date', 'total_spent']).copy()
        self.single_day_data = pd.DataFrame({
            'date': [datetime(2023, 1, 1)],
            'total_spent': [100],
            'day_of_week': [1]
        }).copy()
    
    # Test predict_next_day
    def test_predict_next_day_normal(self):
        result = predict_next_day(self.daily_data)
        self.assertIsInstance(result, float)
        self.assertGreaterEqual(result, 0)
    
    def test_predict_next_day_empty(self):
        result = predict_next_day(self.empty_data)
        self.assertEqual(result, 0.0)
    
    def test_predict_next_day_single_value(self):
        result = predict_next_day(self.single_day_data)
        self.assertEqual(result, 100.0)
    
    # Test predict_next_week
    def test_predict_next_week_normal(self):
        result = predict_next_week(self.daily_data)
        self.assertIsInstance(result, float)
        self.assertGreaterEqual(result, 0)
    
    def test_predict_next_week_empty(self):
        result = predict_next_week(self.empty_data)
        self.assertEqual(result, 0.0)
    
    # Test predict_next_month
    def test_predict_next_month_normal(self):
        result = predict_next_month(self.daily_data)
        self.assertIsInstance(result, float)
        self.assertGreaterEqual(result, 0)
    
    def test_predict_next_month_empty(self):
        result = predict_next_month(self.empty_data)
        self.assertEqual(result, 0.0)
    
    def test_predict_next_month_short_data(self):
        # Create a proper copy to avoid warnings
        short_data = self.daily_data.iloc[:5].copy()
        result = predict_next_month(short_data)
        expected = short_data['total_spent'].mean() * 30
        self.assertAlmostEqual(result, expected, delta=0.1)
    
    # Test predict_category_spending
    def test_predict_category_spending_day(self):
        results = predict_category_spending(self.category_data, 'day')
        for cat, val in results.items():
            self.assertIsInstance(val, float)
            self.assertGreaterEqual(val, 0)
    
    def test_predict_category_spending_week(self):
        results = predict_category_spending(self.category_data, 'week')
        for cat, val in results.items():
            self.assertIsInstance(val, float)
            self.assertGreaterEqual(val, 0)
    
    def test_predict_category_spending_month(self):
        results = predict_category_spending(self.category_data, 'month')
        for cat, val in results.items():
            self.assertIsInstance(val, float)
            self.assertGreaterEqual(val, 0)
    
    def test_predict_category_empty(self):
        results = predict_category_spending({}, 'day')
        self.assertEqual(results, {})
    
    # Test _predict_next_day_category
    def test_predict_next_day_category(self):
        for cat, df in self.category_data.items():
            result = _predict_next_day_category(df)
            self.assertIsInstance(result, float)
            self.assertGreaterEqual(result, 0)
    
    # Test _predict_next_week_category
    def test_predict_next_week_category(self):
        for cat, df in self.category_data.items():
            result = _predict_next_week_category(df)
            self.assertIsInstance(result, float)
            self.assertGreaterEqual(result, 0)
    
    # Test _predict_next_month_category
    def test_predict_next_month_category(self):
        for cat, df in self.category_data.items():
            result = _predict_next_month_category(df)
            self.assertIsInstance(result, float)
            self.assertGreaterEqual(result, 0)
